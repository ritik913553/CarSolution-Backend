...
const placeBid = async (req, res) => {
  try {
    const salespersonId = req.user._id;
    const { postId, bidAmount, offerDetails } = req.body;
    ...
    const existingBidIndex = post.bid.findIndex(b => b.user.equals(salespersonId));
    if (existingBidIndex !== -1) {
      return res.status(400).json({ message: "You already placed a bid on this post" });
    }
